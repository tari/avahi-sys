...
fn whitelist_from_file<F>(mut builder: bindgen::Builder, name: &str, mut f: F) -> bindgen::Builder
        where F: FnMut(bindgen::Builder, String) -> bindgen::Builder {

    let infile: PathBuf = env::var_os("CARGO_MANIFEST_DIR").unwrap().into();
    let infile = File::open(infile.join(name))
        .expect("Failed to open whitelist file");
...
fn main() {
    ...
    bindings = whitelist_from_file(bindings, "functions.txt", bindgen::Builder::whitelisted_function);
